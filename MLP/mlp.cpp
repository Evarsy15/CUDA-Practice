...
inline void MLP::match_single(double *res, double *ans, int output_size, int &val) {
    int pos1 = distance(res, max_element(res, res + output_size));
    int pos2 = distance(ans, max_element(ans, ans + output_size));
    val += (pos1 == pos2);
}

void MLP::match(double *res, vector<double*> &ans, int num_data, int output_size, int &val) {
    for (int i = 0; i < num_data; i++)
        match_single(res + (i * output_size), ans[i], output_size, val);
}
